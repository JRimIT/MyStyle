...
export const verifyUserOrRedirect = (req, res, next) => {
    ...
    if (!token) {
        if (isWebPage) {
            if (req.session)
                req.session.flash = { type: 'error', message: 'Vui lòng đăng nhập để sử dụng tính năng này!' };
    ...
    jwt.verify(token, process.env.JWT_SECRET, (err, decoded) => {
        if (err) {
            ...
            return res
                .status(401)
                .json({ success: false, loginRequired: true, message: 'Token expired. Please login.' });
        }
        req.user = decoded;
        next();
    });
