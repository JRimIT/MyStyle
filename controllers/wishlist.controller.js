...
export async function addToWishlist(req, res) {
    try {
        ...
        if (!userId)
            return res
                .status(401)
                .json({ success: false, loginRequired: true, message: 'Token required. Please login.' });
        ...
        if (req.headers.accept?.includes('application/json')) return res.json({ ok: true, count: wl.items.length });
