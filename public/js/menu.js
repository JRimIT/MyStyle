...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function renderPagination(totalItems, page) {
        ...
        container.appendChild(
            createBtn('>', page >= pages, () => renderProducts(currentList, Math.min(pages, page + 1))),
        );
    }
    ...
    function showAddToast(item) {
      ...
      <div class="d-flex align-items-start">
        ...
        <div>
          ...
          <div class="mt-1">Tổng: <span class="text-danger">${((item.price || 0) * item.qty).toLocaleString(
              'vi-VN',
          )}₫</span></div>
          <div class="mt-3">
    ...
    function updateActiveFiltersUI() {
        ...
        if (cats.length || pris.length || sizs.length) {
            ...
            clearBtn.addEventListener('click', () => {
                // clear all filters
                document
                    .querySelectorAll('#categories-ao input, #categories-quan input, input[name="price"]')
    ...
    function removeFilter(type, value) {
        if (type === 'category') {
            ...
            const btn = Array.from(document.querySelectorAll('.size-btn')).find(
                (b) => b.dataset.size === value || b.textContent.trim() === value,
            );
